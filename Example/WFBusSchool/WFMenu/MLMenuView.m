...
{
    self = [super initWithFrame:CGRectMake(0, NavHeight, k_ScreenWidth, k_ScreenHeight-NavHeight-TabbarHeight)];
    if (self) {
...
- (void)setSubViews
{
    [[[YFKeyWindow shareInstance] getCurrentVC].view addSubview:self];
...
- (void)showMenuEnterAnimation:(MLEnterAnimationStyle)animationStyle
{
   ...
   _animationStyle = animationStyle;
    ...
    if (animationStyle == MLEnterAnimationStyleRight) {
        ...
        self.contentView.transform = CGAffineTransformMakeScale(1, 0.6);
...
- (void)hidMenuExitAnimation:(MLEnterAnimationStyle)animationStyle
{
    ...
    if (_animationStyle == MLEnterAnimationStyleRight) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            self.contentView.alpha = 0;
            self.contentView.transform = CGAffineTransformMakeScale(1, 0.8);
        } completion:^(BOOL finished) {
...
{
    !self.dissaperBlock ? : self.dissaperBlock();
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    !self.didSelectBlock ? : self.didSelectBlock(indexPath.row);
